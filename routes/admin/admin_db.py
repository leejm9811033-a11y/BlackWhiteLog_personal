...
def approve_restaurant_request(request_id):
    ...
    try:
        with conn.cursor() as cur:
            ...
            # 0-1. category_name으로 restaurant_category_id 찾기
            cur.execute("""
            ...
            if not category_row:
                raise Exception(f"카테고리를 찾을 수 없습니다: {req['category_name']}")

            restaurant_category_id = category_row["restaurant_category_id"]

            # 1. users 테이블에서 유저 정보 조회
            cur.execute("""
                SELECT
                    user_id,
                    email,
